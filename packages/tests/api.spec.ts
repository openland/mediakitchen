...
describe('api', () => {
    ...
    it('should create plain transport', async () => {
        ...
        // Create transport
        let transport1 = await api1.createPlainTransport(router1, { listenIp: '127.0.0.1' }, 'transport1');
        let transport2 = await api2.createPlainTransport(router2, { listenIp: '127.0.0.1' }, 'transport2');

        // Double invoke
        let transport3 = await api1.createPlainTransport(router1, { listenIp: '127.0.0.1' }, 'transport1');
        let transport4 = await api2.createPlainTransport(router2, { listenIp: '127.0.0.1' }, 'transport2');
        expect(transport3.id).toBe(transport1.id);
        expect(transport4.id).toBe(transport2.id);
