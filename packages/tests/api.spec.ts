...
describe('api', () => {
    ...
    it('should create plain transport', async () => {
        ...
        // Create transport
        let transport1 = await api1.createPlainTransport(router1, { enableSrtp: true }, 'transport1');
        let transport2 = await api2.createPlainTransport(router2, { enableSrtp: true }, 'transport2');

        // Double invoke
        let transport3 = await api1.createPlainTransport(router1, { enableSrtp: true }, 'transport1');
        let transport4 = await api2.createPlainTransport(router2, { enableSrtp: true }, 'transport2');
        expect(transport3.id).toBe(transport1.id);
        expect(transport4.id).toBe(transport2.id);

        // Connect
        await api1.connectPlainTransport({ id: transport1.id, ip: transport2.tuple.localIp, port: transport2.tuple.localPort, srtpParameters: transport2.srtpParameters! });
        await api2.connectPlainTransport({ id: transport2.id, ip: transport1.tuple.localIp, port: transport1.tuple.localPort, srtpParameters: transport1.srtpParameters! });

        // Double invoke
        await api1.connectPlainTransport({ id: transport1.id, ip: transport2.tuple.localIp, port: transport2.tuple.localPort });
        await api2.connectPlainTransport({ id: transport2.id, ip: transport1.tuple.localIp, port: transport1.tuple.localPort });

        // Close
        await api1.closePlainTransport(transport1.id);
        await api2.closePlainTransport(transport2.id);
    ...
    it('should create pipe transport', async () => {
        jest.setTimeout(10000);
        const nc = await connect({ payload: Payload.JSON });
        const connectionInfo: ConnectionInfo = { nc, rootTopic: randomKey() };
        const worker1 = await createWorker({ connectionInfo });
        ...
        // Create transport
        let transport1 = await api1.createPipeTransport(router1, { enableSrtp: true }, 'transport1');
        let transport2 = await api2.createPipeTransport(router2, { enableSrtp: true }, 'transport2');

        // Double invoke
        let transport3 = await api1.createPipeTransport(router1, { enableSrtp: true }, 'transport1');
        let transport4 = await api2.createPipeTransport(router2, { enableSrtp: true }, 'transport2');
        expect(transport3.id).toBe(transport1.id);
        expect(transport4.id).toBe(transport2.id);

        console.warn(transport1);
        console.warn(transport2);

        // Connect
        await api1.connectPipeTransport({ id: transport1.id, ip: transport2.tuple.localIp, port: transport2.tuple.localPort, srtpParameters: transport2.srtpParameters! });
        await api2.connectPipeTransport({ id: transport2.id, ip: transport1.tuple.localIp, port: transport1.tuple.localPort, srtpParameters: transport1.srtpParameters! });

        // Double invoke
        await api1.connectPipeTransport({ id: transport1.id, ip: transport2.tuple.localIp, port: transport2.tuple.localPort });
        await api2.connectPipeTransport({ id: transport2.id, ip: transport1.tuple.localIp, port: transport1.tuple.localPort });

        // Close
        await api1.closePipeTransport(transport1.id);
        await api2.closePipeTransport(transport2.id);
